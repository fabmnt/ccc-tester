...
5. Wire the client: ConvexAuthProvider, the sign-in component, and route guards. If you import UI primitives (button, input, textarea, label, and so on), add them first with `pnpm dlx bejamas@latest add <name>`; a missing @/components/ui/* is a hard build error.
...
- Install any UI primitive you import up front (`pnpm dlx bejamas@latest add ...`); a missing @/components/ui/* is a hard build failure.
