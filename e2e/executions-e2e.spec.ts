...
test.describe("CCCdashboard executions", () => {
      ...
      try {
        ...
        if (updated) {
          try {
            ...
            if (
              pairedResult &&
              (await cellText(pairedResult.cell)) !== pairedResult.originalValue
            ) {
  ...
  test("applies specific column filters and general filters", async ({
    ...
    await panel.getByRole("button", { name: "Clean all", exact: true }).click();
    ...
    for (const filterDefinition of GENERAL_FILTERS) {
      ...
      const columnIndex = await findHeaderIndex(page, filterDefinition.header);
